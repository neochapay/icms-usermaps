...
$ROOT/images/fotolib/$Размер_$MD5оригинала.jpg
Размер может быть:
S - превью 
L - размер для показа
...
//Если мы вошли и всем разрешено добавлять фото    
    if($this->inUser->id != 0 AND $cfg['maps_image_acces'] == "all") 
    {
      $allow_add_foto = TRUE;
    }
//Если мы автор и разрешено добавлять только авторам
    if($place['user_id'] == $this->inUser->id AND $cfg['maps_image_acces'] == "author")
    {
      $allow_add_foto = TRUE;
    }
//Если разрешено добавлять только админам
    if($this->inUser->is_admin AND $cfg['maps_image_acces'] == "admin")
    {
      $allow_add_foto = TRUE;
    }
